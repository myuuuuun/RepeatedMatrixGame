...
class RepeatedMatrixGame(object):
    ...
    def play(self, **kwargs):
            ...
            for j, str2 in enumerate(self.strategies):
                if i != j:
                    ...
                    for r in range(self.repeat):
                        ...
                        if record:
                            if i < j:
                                set_result[r, i*(strlen-1)+(j-1)] += s1 / ts_list[r]
                                set_result[r, j*(strlen-1)+i] += s2 / ts_list[r]
                            else:
                                set_result[self.repeat+r, i*(strlen-1)+j] += s1 / ts_list[r]
                                set_result[self.repeat+r, j*(strlen-1)+(i-1)] += s2 / ts_list[r]
        ...
        for i in range(strlen):
            sum_session_ave[i] = np.sum(session_average[i]) / (strlen-1)
            sum_total_ave[i] = np.sum(total_average[i]) / (strlen-1)
        ...
        for i in range(strlen):
            s = ranking_session[i]
            print("{0}. \"{1}\" -> セッションを重率1で平均: {2:.3f}, ステージゲームを重率1で平均: {3:.3f}"
                .format(i+1, self.str_name(self.strategies[s]), sum_session_ave[s], sum_total_ave[s]))
        ...
        if record:
            ...
            tuples1 = list(itertools.permutations(array1, 2))
